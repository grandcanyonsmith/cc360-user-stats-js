...
// //             <TableCell>{formatDemoCompleted(user.demo_call)}</TableCell>
// //             <TableCell>{formatHasOnboardingCallScheduled(user.onboarding_call)}</TableCell>
// //             <TableCell>{formatOnboardingCompleted(user.onboarding_call)}</TableCell>
...
//       return (
//         <TableRow key={user.location_id} className="flex justify-between w-full">
//           <TableCell className="flex-1 text-[10px]">
//             <div className="flex items-center">
//               <a href={`https://app.coursecreator360.com/v2/location/${user.location_id}/dashboard`} className="text-indigo-600 hover:text-indigo-900 location-name">
//                 {user.location_name.replace("'s Account", '').replace('Account', '').trim().length > 15 ? `${user.location_name.replace("'s Account", '').replace('Account', '').trim().substring(0, 15)}...` : user.location_name.replace("'s Account", '').replace('Account', '').trim()}
//               </a>
...
//             <TableCell className="flex-1 text-[10px] ml-8">{formatHasDemoCallScheduled(user.demo_call)}</TableCell>
//           )}
//           {activeTab !== 'Customer Support' && (
//             <TableCell className="flex-1 text-[10px] ml-8">
...
//           {activeTab !== 'Sales' && (
//             <>
//               <TableCell className="flex-1 text-[10px] ml-8">{formatHasOnboardingCallScheduled(user.onboarding_call)}</TableCell>
//               <TableCell className="flex-1 text-[10px] ml-8">
...
//               </TableCell>
//               <TableCell className="flex-1 text-[10px] ml-8"><span className={`text-[10px] font-semibold ${user.mailgun_connected ? 'text-green-600 bg-green-100' : 'text-red-600 bg-red-100'} rounded-full`}>{formatMailgunConnected(user.mailgun_connected)}</span></TableCell>
//               <TableCell className="flex-1 text-[10px] ml-auto"><span className={`text-[10px] font-semibold ${user.payment_processor_integration ? 'text-green-600 bg-green-100' : 'text-red-600 bg-red-100'} rounded-full`}>{formatPaymentProcessor(user.payment_processor_integration)}</span></TableCell>
//             </>
//           )}
//           {activeTab === 'Sales' && (
//             <TableCell className="flex-1 text-[10px] ml-auto">{formatHasOnboardingCallScheduled(user.onboarding_call)}</TableCell>
//           )}
//         </TableRow>
//       );
//     });
...
//                 className={`text-xs font-semibold px-4 py-2 rounded-md ${activeTab === tab ? 'bg-black text-white' : 'bg-gray-200 text-gray-900 hover:bg-gray-300'}`}
//               >
//                 {tab}
//               </button>
//             ))}
...
//                 <StatCard title="Daily MRR" value={formatCurrency(avgDailyMRR)} subtitle={`Average Daily increase`} trendData={revenueTrendData} showGraph={true} isPercentage={false} />
//                 <StatCard title="MRR Trajectory" value={formatCurrency(mrrTrajectory)} subtitle={`Projected monthly increase`} trendData={revenueTrendData} showGraph={true} isPercentage={false} />
//                 <StatCard title="Demo 📆" value={stats.demoCallPercentage} subtitle={stats.demoCallCount} trendData={aggregateDataByDay(filteredUsers, 'demo_call')} showGraph={true} isPercentage={true} />
//                 <StatCard title="Onboarding 📆" value={stats.onboardingCallPercentage} subtitle={stats.onboardingCallCount} trendData={aggregateDataByDay(filteredUsers, 'onboarding_call')} showGraph={true} isPercentage={true} />
...
//                 <TableHeader className="flex-1 text-[10px]">Location Name</TableHeader>
//                 {activeTab !== 'Customer Support' && (
//                   <TableHeader className="flex-1 text-[10px] cursor-pointer ml-8" onClick={() => handleSort('demo_call')}>Demo<br />Scheduled</TableHeader>
//                 )}
//                 {activeTab !== 'Customer Support' && (
//                   <TableHeader className="flex-1 text-[10px] ml-8">Demo<br />Completed</TableHeader>
//                 )}
//                 {activeTab !== 'Sales' && (
//                   <>
//                                         <TableHeader className="flex-1 text-[10px] cursor-pointer ml-8" onClick={() => handleSort('onboarding_call')}>Onboarding<br />Scheduled</TableHeader>
//                     <TableHeader className="flex-1 text-[10px] ml-8">Onboarding<br />Completed</TableHeader>
//                     <TableHeader className="flex-1 text-[10px] cursor-pointer ml-8" onClick={() => handleSort('mailgun_connected')}>Mailgun</TableHeader>
//                     <TableHeader className="flex-1 text-[10px] cursor-pointer ml-8" onClick={() => handleSort('payment_processor_integration')}>Pay<br />Int</TableHeader>
//                   </>
//                 )}
//                 {activeTab === 'Sales' && (
//                   <TableHeader className="flex-1 text-[10px] cursor-pointer ml-8" onClick={() => handleSort('onboarding_call')}>Onboarding<br />Scheduled</TableHeader>
...
//       <style jsx>{`
//         th, td {
//           padding: 0.5rem 1rem; /* Adjust padding to make columns less spaced apart */
//         }
//         th {
//           font-size: 0.875rem; /* Smaller text for table headers */
//           white-space: normal; /* Allow text to wrap onto two lines */
//         }
//         .location-name {
//           font-size: 0.875rem; /* Smaller text for location name */
...
//           }
//           th, td {
//             font-size: 0.5rem; /* Smaller text for table data */
//           }
//           .hidden {
//             display: none;
//           }
